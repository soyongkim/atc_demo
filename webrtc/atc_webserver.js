						...
						janus.attach(
							{
								...
								success: function(pluginHandle) {
									...
									$('#naip-btn').click(toggleNetworkanalyzer);
									$('#username').focus();
								...
								webrtcState: function(on) {
									...
									Janus.log("No limit bandwidth & mute start");
									...
									$('#bitrate a').click(function() {
										...
										// 이것도 RTCP의 한 종류인 REMB(Receiver Estimated Maximum Bitrate) 패킷 종류를 이용하여 제한거는거구나!
								...
								onmessage: function(msg, jsep) {
									...
									if(jsep) {
										...
										if(mystream && mystream.getVideoTracks() && mystream.getVideoTracks().length > 0 && !video) {
											...
											// Hide the webcam videocontent hint
											$('#myvideo').hide();
											$('#videolocal').append(
									...
									Janus.log("The DataChannel is available!(local)");
								},
								ondata: function(data) {
									var json = JSON.parse(data);
									Janus.log("We got data from the DataChannel!(local)" + json);
								},
								onlocaltrack: function(track, on) {
									Janus.debug("Local track " + (on ? "added" : "removed") + ":", track);
									// We use the track ID as name of the element, but it may contain invalid characters
									var trackId = track.id.replace(/[{}]/g, "");
									// console.log("Content Hint --- local track");
									...
									if(track.kind === "audio") {
										...
										// $('#videolocal').append('<span class="label label-info hide" id="localid" style="position: relative; top: 0px; right: 0px; margin: 15px;"></span>');
										// $('#localid').removeClass('hide').show();
										// $('#localid').text(myRealID);
										console.log(trackId + " " + stream);
...
// function sendDataToNetworkAnalyzer(id, name) {
// 	//console.log("Send bitrate with id");
// 	// const res = await fetch('https://192.168.93.13:50000/', {
// 	// 	method: 'POST',
// 	// 	headers: {
...
// 	var data = {};
// 	data.rlaten = last_ping_time.toFixed(0);
// 	data.alaten = avg_ping_time.toFixed(0);
// 	data.loss = packet_loss.toFixed(0);
// 	data.bitrate = curBitrate.split(' ')[0];

// 	Janus.log("Send data to Network Analyzer : ", data);

// 	Janus.httpAPICall("https://" + window.location.hostname + ":50000/update", {
// 		verb: 'POST',
// 		body: { "id": id,
// 				"name" : name,
// 				"data" : data, },
...
// function deleteDataSetInNetworkAnalyzer(id) {
// 	Janus.httpAPICall("https://" + window.location.hostname + ":50000/delete", {
// 		verb: 'POST',
// 		body: {
// 			"id": id
// 		},
...
var isRegisteringNetworkanalyzer = false;
var isSendingData = false;
var networkAnalyzerInterval;

function toggleNetworkanalyzer() {
    var naip = $('#naip').val();
    if (naip === "") {
        $('#na-label')
            .removeClass().addClass('label label-warning').html("Insert Network Analyzer IP");
        setTimeout(function () {
            $('#na-label').html("");
        }, 3000);
        return;
    }
    // Check if naip is in IPv4 format
    var ipv4Regex = /^(\d{1,3}\.){3}\d{1,3}$/;
    if (!ipv4Regex.test(naip)) {
        $('#na-label')
            .removeClass().addClass('label label-warning').html("Invalid Network Analyzer IP");
        return;
    } else {
		if (!isRegisteringNetworkanalyzer) {
			// Register to Network Analyzer
			registerNetworkanalyzer(naip);

			// If failed to register, return
			if(!isRegisteringNetworkanalyzer) {
				return;
			}
		}
        ...
        if (!isSendingData) {
            // Start sending data to Network Analyzer
            $('#na-label')
                .removeClass().addClass('label label-success').html("Send data to Network Analyzer");
            setTimeout(function () {
                $('#na-label').html("");
            }, 3000);
            $('#naip-btn').html("Pause"); // Change button text to Pause
            sendNetworkanlyzer(naip);
            isSendingData = true; // Set flag to true
        } else {
            // Stop sending data
            pauseNetworkanalyzer();
        }
    }
}

function registerNetworkanalyzer(naip) {
	console.log("Start registering to Network Analyzer");
	isRegisteringNetworkanalyzer = true; // Set flag to true

	 // Register to Network Analyzer
	 Janus.httpAPICall("https://" + naip + ":50000/register_webrtc", {
        verb: 'POST',
        body: { "id": myRealID },
        success: function(json) {
            Janus.debug(json);
            console.log("Registration successful. Start sending data to Network Analyzer");
        },
        error: function(textStatus, errorThrown) {
            Janus.error(textStatus + ":", errorThrown);
            console.log("Error occurred during registration. Pausing data transmission to Network Analyzer");
            pauseNetworkanalyzer();
            isRegisteringNetworkanalyzer = false; // Set flag to false
        }
    });
}


function sendNetworkanlyzer(naip) {
    console.log("Start sending data to Network Analyzer");

    // Start sending data every second
    networkAnalyzerInterval = setInterval(function() {
        console.log("Send data to Network Analyzer");

        Janus.httpAPICall("https://" + naip + ":50000/update_webrtc", {
            verb: 'POST',
            body: { "id": myRealID, "metrics" : network_status, },
            success: function(json) {
                Janus.debug(json);
            },
            error: function(textStatus, errorThrown) {
                ...
                // Stop sending data and update UI when an error occurs
                pauseNetworkanalyzer();
            }
        });
...
function pauseNetworkanalyzer() {
    console.log("Pause data transmission to Network Analyzer");

    // Stop sending data
    if (networkAnalyzerInterval) {
        clearInterval(networkAnalyzerInterval);
        networkAnalyzerInterval = null;
    }

    // Update UI
    $('#na-label')
        .removeClass().addClass('label label-danger').html("Data transmission paused");
    setTimeout(function () {
        $('#na-label').html("");
    }, 3000);
    $('#naip-btn').html("Send"); // Change button text back to Send data
    isSendingData = false; // Set flag to false
}

window.addEventListener('beforeunload', function (e) {
    // Your clean-up code here
    // For example, you might want to pause the network analyzer when the user leaves the page
    exitNetworkanalyzer();

    // If you want to ask the user for confirmation before leaving the page
    // e.preventDefault();
    // e.returnValue = '';
});

function exitNetworkanalyzer() {
    var naip = $('#naip').val();

    Janus.httpAPICall("https://" + naip + ":50000/exit_webrtc", {
        verb: 'POST',
        body: { "id": myRealID },
        success: function(json) {
            Janus.debug(json);
	...
	sfutest.createOffer(
		{
			...
			media: { data:true, audioRecv: false, videoRecv: false, audioSend: useAudio, videoSend: true, video: "fhdres" },	// Publishers are sendonly
			...
			customizeSdp: function(jsep) {
				...
				// 	if (/^m=video*/.test(str)) {
				// 	  arr[i] += '\r\nb=AS:500';
				// 	  arr[i] += '\r\na=fmtp:100 x-google-max-bitrate=500';
				// 	}
				//   });
				// jsep.sdp = arr.join('\r\n');
				//console.log("Final SDP publishOwnFeed ----");
			...
			onmessage: function(msg, jsep) {
				...
				if(msg["error"]) {
					...
					if(event === "attached") {
						...
						Janus.log("Successfully attached to feed in room " + msg["room"] + " : " + display);
						$('#remote'+remoteFeed.rfindex).removeClass('hide').html(remoteFeed.rfdisplay).show();
						$('#network').removeClass('hide').html(remoteFeed.rfdisplay).show();
						//$('#remote'+remoteFeed.rfindex).removeClass('hide').html(display).show();
					} else if(event === "event") {
				...
				if(jsep) {
					...
					// Answer and attach
					console.log("for creating remote peer data channel");
					remoteFeed.createAnswer(
						{
							...
							customizeSdp: function(jsep) {
								...
								//   });
								// jsep.sdp = arr.join('\r\n');
								//console.log("Final SDP remoteFeed.createAnswer ----");
								//console.log(jsep.sdp);
							},
							success: function(jsep) {
				...
				Janus.log("The DataChannel is available(Peer)!");
			},
			ondata: function(data) {
				//~ $('#datarecv').val(data);
				var json = JSON.parse(data);
				Janus.log("We got data from the DataChannel!(Peer)");
				var type = json["type"];
				console.log(json);
				if(type == "echo_ping") {
					var msg = json["sentNum"];
					var rid = json["id"];
					received_ping(msg, rid);
				} else if(type == "ping") {
					echo_ping(json);
				}
			...
			onremotetrack: function(track, mid, on) {
				...
				if(track.kind === "audio") {
					...
					$('#videoremote'+remoteFeed.rfindex).append(
						...
						'<span class="label label-info hide" id="curid'+remoteFeed.rfindex+'" style="position: absolute; top: 0px; right: 0px; margin: 15px;"></span>');
					Janus.attachMediaStream($('#remotevideo' + remoteFeed.rfindex + '-' + mid).get(0), stream);
					// Note: we'll need this for additional videos too
					if(!bitrateTimer[remoteFeed.rfindex]) {
						$('#curbitrate'+remoteFeed.rfindex).removeClass('hide').show();
						$('#curid'+remoteFeed.rfindex).removeClass('hide').show();
						$('#curid'+remoteFeed.rfindex).text(id);
						bitrateTimer[remoteFeed.rfindex] = setInterval(function() {
							if(!$("#videoremote" + remoteFeed.rfindex + ' video').get(0))
								return;
							// Display updated bitrate, if supported
							network_status = remoteFeed.getStatus();
							network_status.local_id = myRealID;
							var bitrate = network_status.throughput;

							//console.log("display: ", display);
							//var bitrate = remoteFeed.getBitrate();

							curBitrate = bitrate;
							$('#curbitrate'+remoteFeed.rfindex).text(bitrate + ' kbits/sec');
							//sendDataToNetworkAnalyzer(id, display);
							// Check if the resolution changed too
							//console.log($("#videoremote" + remoteFeed.rfindex + ' video').get(0));
							...
							console.log("Status:", network_status);
						}, 1000);
					}
...
function start_ping(display) {
    ...
    last_ping_time = 0;
    ping_times = {};
    ping_to = setInterval(function() {
		send_ping(display);
	}, ping_interval);
}

function send_ping(display) {
	var message = {
		type: "ping",
		id: myRealID,
		to: display,
...
function echo_ping(packet) {
	var message = {
		type: "echo_ping",
		id: myRealID,
		sentNum: packet.sentNum,
	};
	sfutest.data({
		text: JSON.stringify(message),
		error: function (reason) { bootbox.alert(reason); },
...
function received_ping(msg, id) {
    ...
    avg_ping_time = total_ping_time/pings_received;
    packet_loss = (pings_sent-pings_received)/pings_sent*100;
    $('#pings_sent').html(pings_sent);
	$('#pings_received').html(pings_received);
    $('#average_ping_time').html(avg_ping_time.toFixed(0));
	$('#recent_ping_time').html(last_ping_time.toFixed(0));
    $('#packet_loss').html(packet_loss.toFixed(0));
	$('#network_throughput').html(curBitrate);
	//console.log("["+ id +"] latency : " + last_ping_time + " msec  / pass loss rate:" + packet_loss + "%");
}
